...
models = {
    ...
    "SiCNN wide, k=5, r=2^(1/3), n=6, D=0":SiCNN(f_in = 3, size = 5, ratio = 2**(1/3), nratio = 6, srange = 0, factor = 3.0),
    ...
    "SiCNN wide, k=5, r=2^(2/3), n=3, D=0": SiCNN(f_in = 3, size = 5, ratio = 2**(2/3), nratio = 3, srange = 0, factor=2.23)
}
...
for m, name in enumerate(models):
    ...
    for epoch in range(1, nb_epochs + 1): 
        train_l, train_a = train(models[name], train_loader, learning_rate, criterion, epoch, batch_log, device) 
        train_l, train_a = test(models[name], train_loader, criterion, epoch, batch_log, device) 

    pickle.dump(models[name], open("trained_model_{}.pickle".format(m), "wb"))
    ...
    for ii in range(repeats):
        ...
        for s in scales: 
            test_transf = transforms.Compose([
                                RandomRescale(size = 32, scales = (s, s), sampling = "uniform"),  #apply scaling uniformly 
                                transforms.ToTensor(), transforms.Normalize((0.1307,),(0.3081,))])
            ...
            test_l, test_a = test(models[name], test_loader, criterion, epoch, batch_log, device)
            test_acc.append(test_a)
        m_test_acc.append(test_acc)
    avg_test_acc = np.mean(np.array(m_test_acc), axis=0)    
    std_test_acc = np.mean(np.std(m_test_acc), axis=0)    
